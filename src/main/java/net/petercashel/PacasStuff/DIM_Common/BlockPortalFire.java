...
public class BlockPortalFire extends BlockFire {
	...
	public BlockPortalFire(String blockName)
	{
		super();
	...
	{
		if(par1World.provider.dimensionId == 1 || !mod_PacasStuff.WOPPortal.func_150000_e(par1World, par2, par3, par4)){
		...
		if(par1World.provider.dimensionId == 1 || !mod_PacasStuff.RedlandsPortal.func_150000_e(par1World, par2, par3, par4)){
			if (!World.doesBlockHaveSolidTopSurface(par1World, par2, par3 - 1, par4) && !this.canNeighborBurn(par1World, par2, par3, par4))
			{
				par1World.setBlockToAir(par2, par3, par4);
			}
	...
	public void randomDisplayTick(World par1World, int par2, int par3, int par4, Random par5Random)
	{
		...
		if ((!par1World.doesBlockHaveSolidTopSurface(par1World, par2, par3 - 1, par4)) && (!((BlockPortalFire) mod_PacasStuff.PortalFire).canBlockCatchFire(par1World, par2, par3 - 1, par4, ForgeDirection.UP)))
		{
			if (((BlockPortalFire) mod_PacasStuff.PortalFire).canBlockCatchFire(par1World, par2 - 1, par3, par4, ForgeDirection.EAST))
			{
				for (int l = 0; l < 2; l++)
				{
			...
			if (((BlockPortalFire) mod_PacasStuff.PortalFire).canBlockCatchFire(par1World, par2 + 1, par3, par4, ForgeDirection.WEST))
			{
				for (int l = 0; l < 2; l++)
				{
			...
			if (((BlockPortalFire) mod_PacasStuff.PortalFire).canBlockCatchFire(par1World, par2, par3, par4 - 1, ForgeDirection.SOUTH))
			{
				for (int l = 0; l < 2; l++)
				{
			...
			if (((BlockPortalFire) mod_PacasStuff.PortalFire).canBlockCatchFire(par1World, par2, par3, par4 + 1, ForgeDirection.NORTH))
			{
				for (int l = 0; l < 2; l++)
				{
					float f = par2 + par5Random.nextFloat();
			...
			if (((BlockPortalFire) mod_PacasStuff.PortalFire).canBlockCatchFire(par1World, par2, par3 + 1, par4, ForgeDirection.DOWN))
			{
				for (int l = 0; l < 2; l++)
				{
					float f = par2 + par5Random.nextFloat();
	...
	{
		this.blockIcon = par1IconRegister.registerIcon("pacas_stuff" + ":" + "wopFire_0");

		this.iconArray = new IIcon[] { 
				par1IconRegister.registerIcon("pacas_stuff" + ":" + "wopFire_0"), par1IconRegister.registerIcon("pacas_stuff" + ":" + "wopFire_1")
		};
	}
